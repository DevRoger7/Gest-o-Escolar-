<?php
/**
 * Script para analisar habilidades da BNCC
 * Compara a lista fornecida com o banco de dados
 */

// Ler o arquivo SQL para extrair habilidades já cadastradas
...
// Extrair todos os códigos BNCC do arquivo SQL que são do Ensino Fundamental - Anos Iniciais
// Buscar linhas que contêm 'Ensino Fundamental – Anos Iniciais' e extrair os códigos
...
$codigosNoSQL = array_unique($codigosNoSQL);

echo "Códigos encontrados no SQL (Ensino Fundamental - Anos Iniciais): " . count($codigosNoSQL) . "\n";
$codigosNoBanco = $codigosNoSQL; // Usar os códigos do SQL como referência
echo "Códigos no banco de dados: " . count($codigosNoBanco) . "\n\n";

// Lista completa de habilidades fornecida (extraída da mensagem do usuário)
$todasHabilidades = [
    // EF15LP (1-19)
...
$totalFornecidas = count($todasHabilidades);
echo "Total de habilidades fornecidas: $totalFornecidas\n\n";

// Comparar
$codigosNoBancoAssoc = array_flip($codigosNoBanco);
$cadastradas = [];
$faltantes = [];

foreach ($todasHabilidades as $codigo) {
    if (isset($codigosNoBancoAssoc[$codigo])) {
        $cadastradas[] = $codigo;
    } else {
        $faltantes[] = $codigo;
    }
...
$faltantesPorComponente = organizarPorComponente($faltantes);
$cadastradasPorComponente = organizarPorComponente($cadastradas);
...
echo "RESUMO GERAL:\n";
echo "Total de habilidades fornecidas: $totalFornecidas\n";
echo "Total de habilidades no banco: " . count($codigosNoBanco) . "\n";
echo "Habilidades já cadastradas: " . count($cadastradas) . "\n";
echo "Habilidades faltantes: " . count($faltantes) . "\n";
echo "Percentual cadastrado: " . number_format((count($cadastradas) / $totalFornecidas) * 100, 2) . "%\n\n";
...
foreach ($faltantesPorComponente as $componente => $codigos) {
    ...
    // Mostrar apenas os primeiros 10 de cada componente
    $mostrar = array_slice($codigos, 0, 10);
    foreach ($mostrar as $codigo) {
        echo "  - $codigo\n";
    }
    if (count($codigos) > 10) {
        echo "  ... e mais " . (count($codigos) - 10) . " habilidades\n";
    }
    echo "\n";
}
...
$relatorio = "========================================\n";
$relatorio .= "RELATÓRIO COMPLETO - HABILIDADES BNCC\n";
$relatorio .= "Ensino Fundamental - Anos Iniciais\n";
$relatorio .= "Gerado em: " . date('d/m/Y H:i:s') . "\n";
$relatorio .= "========================================\n\n";

$relatorio .= "RESUMO GERAL:\n";
$relatorio .= "Total de habilidades fornecidas: $totalFornecidas\n";
$relatorio .= "Total de habilidades no banco: " . count($codigosNoBanco) . "\n";
$relatorio .= "Habilidades já cadastradas: " . count($cadastradas) . "\n";
$relatorio .= "Habilidades faltantes: " . count($faltantes) . "\n";
$relatorio .= "Percentual cadastrado: " . number_format((count($cadastradas) / $totalFornecidas) * 100, 2) . "%\n\n";

$relatorio .= "========================================\n";
$relatorio .= "LISTA COMPLETA DE HABILIDADES FALTANTES:\n";
$relatorio .= "========================================\n";
$relatorio .= implode(', ', $faltantes) . "\n\n";

$relatorio .= "========================================\n";
$relatorio .= "HABILIDADES FALTANTES POR COMPONENTE:\n";
$relatorio .= "========================================\n\n";

foreach ($faltantesPorComponente as $componente => $codigos) {
    $relatorio .= "--- $componente (" . count($codigos) . " habilidades) ---\n";
    foreach ($codigos as $codigo) {
        $relatorio .= "  - $codigo\n";
    }
    $relatorio .= "\n";
...
$arquivoRelatorio = __DIR__ . '/relatorio_habilidades_bncc_' . date('Y-m-d_H-i-s') . '.txt';
file_put_contents($arquivoRelatorio, $relatorio);

echo "Relatório completo salvo em: $arquivoRelatorio\n";
