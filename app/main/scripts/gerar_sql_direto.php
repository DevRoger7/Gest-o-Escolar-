<?php
/**
 * Script para gerar SQL de inserção das habilidades faltantes
 * Processa diretamente o texto fornecido pelo usuário
 */
...
// Lista de códigos faltantes (do relatório gerado anteriormente)
$codigosFaltantes = [
...
// O texto foi fornecido na mensagem - vou criar um arquivo temporário com ele
// ou processar diretamente se o arquivo existir
...
if (!file_exists($arquivoTexto)) {
    ...
    echo "INSTRUÇÕES:\n";
    echo "========================================\n";
    echo "1. Crie um arquivo chamado 'habilidades_texto_completo.txt'\n";
    echo "2. Na pasta: " . __DIR__ . "\n";
    echo "3. Cole o texto completo fornecido na mensagem do usuário\n";
    echo "4. Execute este script novamente\n";
    echo "\nO texto deve estar no formato:\n";
    echo "EF15LP01 - Descrição., EF15LP02 - Outra descrição., ...\n";
    exit(1);
}
...
// Função melhorada para processar habilidades
function processarHabilidadesMelhorado($texto, $codigosFaltantes) {
    $habilidades = [];
    
    // Para cada código faltante, encontrar sua descrição no texto
    foreach ($codigosFaltantes as $codigo) {
        ...
        if (preg_match($pattern, $texto, $matches)) {
            ...
            if ($posicao !== false) {
                // Pegar tudo após o código até encontrar o próximo código ou fim
                $parte = substr($texto, $posicao);
                $proximoCodigo = preg_match('/EF\d{2}[A-Z]{2}\d{2}/', substr($parte, strlen($codigo) + 1), $nextMatch);
...
echo "Processando descrições...\n";
$habilidadesComDescricao = processarHabilidadesMelhorado($textoCompleto, $codigosFaltantes);

echo "Descrições encontradas: " . count($habilidadesComDescricao) . " de " . count($codigosFaltantes) . "\n";

if (count($habilidadesComDescricao) < count($codigosFaltantes) * 0.8) {
    echo "AVISO: Menos de 80% das descrições foram encontradas.\n";
    echo "Verifique se o texto está completo e no formato correto.\n";
}
...
$sql .= "-- Descrições encontradas: " . count($habilidadesComDescricao) . "\n\n";

$sql .= "START TRANSACTION;\n\n";

$contador = 0;
$contadorComDescricao = 0;
...
foreach ($codigosFaltantes as $codigo) {
    ...
    if (preg_match('/EF(\d{2})([A-Z]{2})(\d{2})/', $codigo, $matches)) {
        ...
        // Obter descrição
        $descricao = isset($habilidadesComDescricao[$codigo]) 
            ? $habilidadesComDescricao[$codigo] 
            : "DESCRIÇÃO NÃO ENCONTRADA - Preencher manualmente";
        
        if (isset($habilidadesComDescricao[$codigo])) {
            $contadorComDescricao++;
        }
...
echo "Descrições encontradas: $contadorComDescricao\n";
echo "Descrições faltantes: " . (count($codigosFaltantes) - $contadorComDescricao) . "\n";
echo "\nVocê pode executar este arquivo SQL no seu banco de dados.\n";
echo "ATENÇÃO: Verifique as descrições marcadas como 'DESCRIÇÃO NÃO ENCONTRADA' e preencha manualmente.\n";
