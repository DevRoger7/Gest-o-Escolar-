...
class DashboardStats {
     ...
     */
    /**
     * Busca atividades recentes do nutricionista
     */
    public function getAtividadesNutricionista($nutricionistaId, $limit = 5) {
        $atividades = [];
        
        try {
            // Cardápios criados recentemente
            $stmt = $this->conn->prepare("
                SELECT c.id, c.mes, c.ano, c.status, c.criado_em, e.nome as escola_nome
                FROM cardapio c
                ...
                LIMIT :limit
            ");
            $stmt->bindValue(':nutricionista_id', $nutricionistaId, PDO::PARAM_INT);
            $stmt->bindValue(':limit', (int)$limit, PDO::PARAM_INT);
            $stmt->execute();
            ...
            foreach ($cardapios as $cardapio) {
                ...
                $mesNome = $meses[$cardapio['mes'] - 1] ?? $cardapio['mes'];
                $atividades[] = [
                    'tipo' => 'cardapio_criado',
                    'titulo' => 'Cardápio criado',
                    'descricao' => ($cardapio['escola_nome'] ?? 'Escola') . ' - ' . $mesNome . '/' . $cardapio['ano'],
                    'data' => $cardapio['criado_em'] ?? date('Y-m-d H:i:s'),
                    'status' => $cardapio['status'] ?? 'RASCUNHO',
                    'icon' => 'document',
                    'color' => 'green',
                    'link' => 'cardapios_nutricionista.php'
                ];
            }
        } catch (Exception $e) {
        ...
        try {
            // Pedidos enviados recentemente
            $stmt = $this->conn->prepare("
                SELECT pc.id, pc.mes, pc.status, pc.data_envio, pc.data_criacao, e.nome as escola_nome
            ...
            foreach ($pedidos as $pedido) {
                ...
                $atividades[] = [
                    'tipo' => 'pedido_enviado',
                    'titulo' => 'Pedido ' . strtolower($pedido['status'] ?? 'ENVIADO'),
                    'descricao' => ($pedido['escola_nome'] ?? 'Escola') . ' - ' . $mesNome,
                    'data' => $pedido['data_envio'] ?? $pedido['data_criacao'] ?? date('Y-m-d H:i:s'),
                    'status' => $pedido['status'] ?? 'ENVIADO',
                    'icon' => 'shopping',
                    'color' => 'orange',
                    'link' => 'pedidos_nutricionista.php'
                ];
            }
        } catch (Exception $e) {
            // Ignorar erro se a tabela não existir
    ...
    /**
     * Conta cardápios pendentes do nutricionista
     */
    public function getCardapiosPendentesNutricionista($nutricionistaId) {
        try {
            $stmt = $this->conn->prepare("
                SELECT COUNT(*) as total 
                FROM cardapio 
                WHERE criado_por = :nutricionista_id AND status = 'RASCUNHO'
            ");
            $stmt->bindValue(':nutricionista_id', $nutricionistaId, PDO::PARAM_INT);
            $stmt->execute();
            $result = $stmt->fetch(PDO::FETCH_ASSOC);
    ...
    /**
     * Conta pedidos pendentes do nutricionista
     */
    public function getPedidosPendentesNutricionista($nutricionistaId) {
        try {
            $stmt = $this->conn->prepare("
                SELECT COUNT(*) as total 
                FROM pedido_cesta 
                WHERE nutricionista_id = :nutricionista_id AND status IN ('RASCUNHO', 'ENVIADO')
            ");
            $stmt->bindValue(':nutricionista_id', $nutricionistaId, PDO::PARAM_INT);
            $stmt->execute();
            $result = $stmt->fetch(PDO::FETCH_ASSOC);
    ...
    /**
     * Busca últimos pedidos do nutricionista
     */
    public function getUltimosPedidosNutricionista($nutricionistaId, $limit = 3) {
        try {
            $stmt = $this->conn->prepare("
                SELECT pc.id, pc.mes, pc.status, e.nome as escola_nome
                FROM pedido_cesta pc
                LEFT JOIN escola e ON pc.escola_id = e.id
                WHERE pc.nutricionista_id = :nutricionista_id
