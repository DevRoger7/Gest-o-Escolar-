...
CREATE TABLE `escola_backup` (
  ...
  `dados_escola` longtext NOT NULL COMMENT 'JSON com todos os dados da escola (backup completo antes da exclusão)',
  `dados_turmas` longtext DEFAULT NULL COMMENT 'JSON com TODOS os dados relacionados (turmas, alunos, notas, frequências, avaliações, boletins, entregas, cardápios, consumo, etc.) - backup completo antes da exclusão',
  `dados_lotacoes` longtext DEFAULT NULL COMMENT 'JSON com dados das lotações (professores, gestores, nutricionistas, funcionários) - backup completo antes da exclusão',
